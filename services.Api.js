...
class ApiService {
  ...
  static _fetchWithRetry(url, options) {
    ...
    const retryDelaySeconds = parseInt(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        Logger.log(
          `ApiService: Đang gọi API, lần thử ${attempt}/${maxRetries}...`,
        );
        ...
        Logger.log(
          `ApiService: Lỗi khi gọi API lần thử ${attempt}: ${e.toString()}`,
        );
        if (attempt >= maxRetries) {
          Logger.log(`ApiService: Đã hết số lần thử lại. Ném lỗi ra ngoài.`);
          throw new Error(
            `Không thể kết nối tới API sau ${maxRetries} lần thử.`,
          );
        }
  ...
  static fetchAllSchedulePages(startDateIso) {
    ...
    Logger.log(
      `ApiService: Bắt đầu lấy lịch học. MSSV: ${studentId}, Ngày BĐ: ${startDateIso}`,
    );
    let allEvents = [];

    try {
      // Gọi trang đầu tiên để lấy tổng số trang
      const initialOptions = this._buildRequestOptions(
        1,
        studentId,
        pageSize,
        startDateIso,
      ...
      if (!initialJson || !initialJson.data || initialJson.data.length < 3) {
        Logger.log(
          "ApiService: Lỗi - API trả về dữ liệu không đúng cấu trúc ở lần gọi đầu tiên.",
        );
        return [];
      }
      ...
      Logger.log(
        `ApiService: Tìm thấy ${totalRecords} buổi học, tương ứng ${totalPages} trang.`,
      );

      // Lặp qua các trang còn lại
      for (let page = 2; page <= totalPages; page++) {
        const pageOptions = this._buildRequestOptions(
          page,
          studentId,
          pageSize,
          startDateIso,
      ...
      Logger.log(
        `ApiService: Hoàn tất. Đã lấy được tổng cộng ${allEvents.length} sự kiện.`,
      );
      const validEvents = allEvents.filter((event) => {
        const status = event.TinhTrang;
        // Giữ lại các buổi học/thi hợp lệ và các buổi được xác định là "Báo nghỉ"
        const validStatuses = [0, 4, 5, 10];
        return validStatuses.includes(status) || event.calenType !== 2;
      });

      Logger.log(
        `ApiService: Sau khi lọc, còn lại ${validEvents.length} sự kiện hợp lệ hoặc báo nghỉ để xử lý.`,
      );
      return validEvents;
    } catch (e) {
      Logger.log(
        `ApiService: LỖI NGHIÊM TRỌNG khi lấy dữ liệu API: ${e.toString()}`,
      );
      // Ném lại lỗi để Controller có thể bắt và thông báo cho người dùng
      throw new Error(
        `Không thể lấy dữ liệu từ máy chủ LHU. Vui lòng thử lại sau. Chi tiết: ${e.message}`,
      );
    }
  }
}
